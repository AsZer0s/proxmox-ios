...
private struct SDNZoneEditor:View{
    ...
    init(item:ProxmoxSDNZone?,onSaved:@escaping()async->Void){self.item=item;self.onSaved=onSaved;_id=State(initialValue:item?.zone ?? "");_type=State(initialValue:item?.type ?? "simple");_nodes=State(initialValue:item?.nodes ?? "");_mtu=State(initialValue:item?.mtu.map(String.init) ?? "");_bridge=State(initialValue:item?.bridge ?? "")}
...
private struct SDNVNetEditor:View{
    ...
    init(item:ProxmoxSDNVNet?,zones:[ProxmoxSDNZone],onSaved:@escaping()async->Void){self.item=item;self.zones=zones;self.onSaved=onSaved;_id=State(initialValue:item?.vnet ?? "");_zone=State(initialValue:item?.zone ?? zones.first?.zone ?? "");_alias=State(initialValue:item?.alias ?? "");_tag=State(initialValue:item?.tag.map(String.init) ?? "");_vlanAware=State(initialValue:item?.vlanAware ?? false)}
